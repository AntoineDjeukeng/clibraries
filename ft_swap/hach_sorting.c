...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/10 19:28:56 by adjeuken          #+#    #+#             */
/*   Updated: 2025/07/10 23:24:48 by adjeuken         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_stack(t_stack *stack)
{
	...
	while (current)
	{
		ft_printf(" %d", current->target_index);
		if (current->next)
			ft_printf(" → ");
...
int	ft_smart_push_b(t_stack *a, int target_position)
{
	...
	short_d = 0;
	tmp = a->head;
	while (tmp && tmp->next && tmp->target_index < target_position
		&& tmp->next->target_index > target_position)
	{
		short_d++;
		tmp = tmp->next;
	}
	short_ind = 0;
	tmp = a->tail;
	while (tmp && tmp->prev && tmp->target_index > target_position
		&& tmp->prev->target_index < target_position)
	{
		short_ind++;
		tmp = tmp->prev;
	}
